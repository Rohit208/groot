...
sub read_file {
	 ...
	 while (my $row=$sth->fetchrow_array())
	   {
		 ...
		 {
		   if(!(${$value}[5] =~ /$row/)) {
	        $sth = $dbh->prepare("select * from Employee_Enquiry where email=?;");
		    $sth->execute($row);
  ...
  close($FILE) or die "couldn't close reader";
    ...
    sub update_details {
        my $sth = $dbh->prepare("update Employee_Enquiry SET name = ?,date_of_birth=?,department=?,date_of_joining=?,salary=? where email=?");
        $sth->execute(@_);
        $sth->finish();
    ...
    sub email_validation {
        ...
        $sth =
          $dbh->prepare("select Email from Employee_Enquiry where Email=?");
        $sth->execute($email);
